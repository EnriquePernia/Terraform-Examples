output "Address" {
  ...
  value       = [for i in docker_container.container[*] : join(" -> ", [i.name], [join(":", ["localhost"], i.ports[*].external)])]
...
output "Container_image" {
  ...
  value       = [for i in docker_container.container[*] : "${i.name} - ${module.image.image_name} - ${terraform.workspace}"]
  description = "Name of Nginx docker container"
}

output "Container_internal_port" {
  value       = [for i in docker_container.container[*] : join(" -> ", ["Internal port"], i.ports[*].internal)]
